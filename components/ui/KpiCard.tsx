...
const accentStyles: Record<string, { card: string; value: string; badge: string }> = {
  ...
  blue:   { card: "border-hebe-navy/30 bg-hebe-navy/5 dark:border-hebe-navy/40 dark:bg-hebe-navy/15",   value: "text-hebe-navy dark:text-gray-300", badge: "bg-hebe-navy/10 dark:bg-gray-800 text-hebe-navy dark:text-gray-300" },
  ...
  gold:   { card: "border-hebe-champagne/40 bg-hebe-champagne/10 dark:bg-gray-800",            value: "text-hebe-ink dark:text-gray-300", badge: "bg-hebe-champagne/20 text-hebe-ink dark:text-gray-300" },
};
...
export function KpiCard({ label, value, sub, accent = "blue", badge }: KpiCardProps) {
  ...
  return (
    <div className={`rounded-xl border p-5 ${s.card}`}>
      ...
      {sub && <p className="mt-1 text-xs text-hebe-ink/40 dark:text-gray-500">{sub}</p>}
      {badge && (
